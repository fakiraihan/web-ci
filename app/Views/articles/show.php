  ...
  <div class="layout-content-container flex flex-col max-w-[960px] flex-1">
    <div class="flex flex-wrap justify-between gap-3 p-4">
      <div class="flex min-w-72 flex-col gap-3">
        <p class="text-white tracking-light text-[32px] font-bold leading-tight"><?= esc($article['title']) ?></p>
        <p class="text-[#a5b6a0] text-sm font-normal leading-normal"><?= esc($article['category']) ?></p>
      </div>
    </div>
    <div class="flex items-center gap-4 bg-[#131712] px-4 min-h-[72px] py-2">
      <div
        ...
        style='background-image: url("<?= esc($article['author_image'] ?? 'https://lh3.googleusercontent.com/aida-public/AB6AXuCipXpga9SNDEogDGipY_9BH0xK1D7qozMM5thKpRLWrGqZUMbE7duAuIHYtUJyCsUqIyCZwmy2f8AQp_GW5_vv9N3YjsURNSN9pOfQE06hvDibOZ5hUt7wiE1F_4NVlGY06aozkONVQ3HGRmNOM8JgTMgmMqmmcZFwQggoEZBZOAzTYG2ICVnthV7p5m61YeeHpaiZa7qtSLp7E9ZbEr0-89W0M3g9NFuVo_js9BdtudbyWQZfIoNlFHI0WxZmKLHDs-1fhCKQBaw') ?>");'
      ></div>
      <div class="flex flex-col justify-center">
        <p class="text-white text-base font-medium leading-normal line-clamp-1"><?= esc($article['author_name']) ?></p>
        <p class="text-[#a5b6a0] text-sm font-normal leading-normal line-clamp-2"><?= date('M d, Y', strtotime($article['created_at'])) ?></p>
      </div>
    </div>
    ...
    <div class="@container">
      <div class="@[480px]:px-4 @[480px]:py-3">
        <div
          ...
          style='background-image: url("<?= esc($article['image_url']) ?>");'
        ></div>
      </div>
    </div>
    <?php endif; ?>
      ...
      <?php if (!empty($comments)): ?>
        <?php foreach ($comments as $comment): ?>
          <div class="flex items-start gap-3 mb-4">
            <div class="bg-center bg-no-repeat aspect-square bg-cover rounded-full size-10 shrink-0" style='background-image: url("https://ui-avatars.com/api/?name=<?= urlencode($comment['username']) ?>&background=2d372a&color=fff")'></div>
            <div>
              <div class="text-white font-semibold text-sm"><?= esc($comment['username']) ?></div>
              <div class="text-[#a5b6a0] text-xs mb-1"><?= date('M d, Y H:i', strtotime($comment['created_at'])) ?></div>
              <!-- VULN: XSS - for research only -->
              <div class="text-white text-base"><?= $comment['content'] ?></div>
              <!-- VULN: IDOR - delete link for any comment -->
              <a href="<?= base_url('/comments/delete/' . $comment['id']) ?>" class="text-red-400 text-xs ml-2">Delete (no check)</a>
            </div>
          </div>
        <?php endforeach; ?>
      <?php endif; ?>
